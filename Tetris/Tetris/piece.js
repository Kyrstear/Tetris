...
Piece.prototype.shift = function(direction) {
  ...
  if (settings.ARR === 0 && this.shiftDelay === settings.DAS) {
    ...
    this.animationState.moving = true;
    this.animationState.prevX = this.x;
...
Piece.prototype.draw = function() {
  ...
  const easingFunction = t => t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t; // Ease in-out quadratic

  if (this.animationState.rotating || this.animationState.moving) {
    this.animationState.progress += 16; // Assuming 60 FPS
    const t = Math.min(this.animationState.progress / animationDuration, 1);
    const easeT = easingFunction(t);

    if (this.animationState.rotating) {
      const prevTetro = pieces[this.index].tetro[this.animationState.prevPos];
      const currentTetro = this.tetro;

      // Draw both previous and current tetromino with opacity
      activeCtx.globalAlpha = 1 - easeT;
      draw(prevTetro, this.x, this.y, activeCtx);
      activeCtx.globalAlpha = easeT;
      draw(currentTetro, this.x, this.y, activeCtx);
      activeCtx.globalAlpha = 1;
    }

    if (this.animationState.moving) {
      const interpolatedX = this.animationState.prevX + (this.x - this.animationState.prevX) * easeT;
      draw(this.tetro, interpolatedX, this.y, activeCtx);
    }

    if (t === 1) {
      this.animationState.rotating = false;
      this.animationState.moving = false;
    }
  } else {
    // Normal drawing when not animating
