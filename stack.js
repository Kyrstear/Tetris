...
Stack.prototype.addPiece = function(tetro) {
  ...
  // Show messages based on lines cleared
  if (linesCleared > 0) {
    combo++;
    let message = '';
    switch (linesCleared) {
      ...
      case 4: 
        message = 'QUAD'; 
        screenShake(200, 10); // Add screen shake for Tetris
        break;
    }
    showCenterMessage(message);
    if (combo > 1) {
      showCenterMessage(`${combo}x`, 1500);
    }
  } else {
    combo = 0;
  }
  ...
  let allClear = true;
  for(let x = 0; x < 10; x++) {
    if(this.grid[x].some(cell => cell !== 0)) {
      allClear = false;
      break;
    }
  }
  if(allClear) {
    showCenterMessage('ALL CLEAR!', 2000);
  }
...
function showCenterMessage(message, duration = 1000) {
  ...
  document.getElementById('b').appendChild(messageElement);
